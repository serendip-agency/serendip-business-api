...
import { InteractionService, CrmService, CrmCheckAccessResultInterface } from "../services";
...
export class InteractionController {
    ...
    constructor() {
        ...
        this.interactionService = Server.services["InteractionService"];
    ...
    public zip: ServerEndpointInterface = {
        ...
        actions: [
            ...
            async (req, res, next, done, access: CrmCheckAccessResultInterface) => {
                ...
                var model = await this.interactionService.find({ crm: access.crm._id.toString(), _vdate: { $gt: range.from, $lt: range.to } });
        ...
        actions: [
            ...
            async (req, res, next, done, access: CrmCheckAccessResultInterface) => {
                ...
                if (req.body._id) {
                    ...
                    var actualRecord = await this.interactionService.findById(req.body._id);
                    if (!actualRecord)
                    ...
                    var changedRecords = _.map(await this.interactionService.find({ crm: access.crm._id.toString(), _vdate: { $gt: range.from, $lt: range.to } }), (item) => {
                        return item._id;
                    });
    ...
    public list: ServerEndpointInterface = {
        ...
        actions: [
            ...
            async (req, res, next, done, access: CrmCheckAccessResultInterface) => {
                ...
                var model = await this.interactionService.findByCrmId(
                    access.crm._id,
                    req.body.skip,
                    req.body.limit);
    ...
    public count: ServerEndpointInterface = {
        ...
        actions: [
            ...
            async (req, res, next, done, access: CrmCheckAccessResultInterface) => {
                ...
                var model = await this.interactionService.count(access.crm._id);
                res.json(model);
    ...
    public insert: ServerEndpointInterface = {
        ...
        actions: [
            ...
            async (req, res, next, done, access: CrmCheckAccessResultInterface) => {
                ...
                var model: InteractionModel = new InteractionModel(req.body);

                try {
                    await InteractionModel.validate(model);
                } catch (e) {
                    return next(new ServerError(400, e.message || e));
                }

                try {
                    model = await this.interactionService.insert(model);
                } catch (e) {
    ...
    public update: ServerEndpointInterface = {
        ...
        actions: [
            ...
            async (req, res, next, done, access: CrmCheckAccessResultInterface) => {
                ...
                try {
                    await this.interactionService.update(model);
                } catch (e) {
                    return next(new ServerError(500, e.message || e));
                }
    ...
    public delete: ServerEndpointInterface = {
        ...
        actions: [
            ...
            async (req, res, next, done, access: CrmCheckAccessResultInterface) => {
                ...
                var interaction = await this.interactionService.findById(_id);
                if (!interaction)
                    return next(new ServerError(400, 'interaction not found'));

                try {
                    await this.interactionService.delete(_id, req.user._id);
                } catch (e) {
                    return next(new ServerError(500, e.message || e));
                }
